...
from torchvision.transforms import Compose, Resize, Normalize, ToTensor, RandomCrop, Grayscale, Lambda
from torchvision.datasets import ImageFolder, MNIST


def make_cmnist_transform(img_size):
    def coloring(x):
        c_idx = torch.randint(0, 3, (1,)).item() 
        c_img = torch.zeros((3, img_size, img_size))
        c_img[c_idx] = x[0]
        return c_img
...
def load_dataset(name, path, img_size=64, batch_size=64, test_ratio=0.1, device='cuda'):
    ...
    elif name in ['shoes', 'handbag']:
        dataset = h5py_to_dataset(path, img_size)
    else:
        raise Exception('Unknown dataset')

    if '8' in name:
        idx = torch.where(dataset.targets == 8)[0].tolist()
    else:
        idx = list(range(len(dataset)))

    if name == 'describable_textures':
        idx = np.random.RandomState(seed=0xBADBEEF).permutation(len(idx)).tolist()
